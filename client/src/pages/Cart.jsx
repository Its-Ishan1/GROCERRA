...
const Cart = () => {
    const { products, Currency, cartItems, removeFromCart, getCartCount, getCartAmount, updateCartItems, navigate, user, axios, setCartItems } = useAppContext();
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            {getCartCount() > 0 ? (
                ...
                <div className="w-full py-24 text-center flex flex-col items-center gap-4 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    <img src={assets.nav_cart_icon} alt="empty cart" className="w-16 opacity-30" />
                    <p className="text-gray-400 text-xl font-medium">Your shopping cart is empty</p>
                    <button
                        onClick={() => navigate("/products")}
                        className="px-8 py-3 bg-primary text-white rounded-full font-medium hover:bg-primary-dull transition"
                    >
                        Browse Products
                    </button>
                </div>
